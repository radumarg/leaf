...
These are Rust style functions clauses that specify pre- & post-conditions that the quantum data should satisfy. This is an attempt to track entanglement at the level of type system, it can be achieved only within some limits. When a condition cannot be checked statically a Lean proof obligation will be generated by the compiler:
...
Stabilized takes as its second argument a list of signed Pauli strings. A more elaborated stabilizer contract example:

...
These are Rust-style functions clauses that specify pre- & post-conditions that the quantum data should satisfy. This is an attempt to track entanglement at the level of the type system, it can be achieved only within some limits. When a condition cannot be checked statically a Lean proof obligation will be generated by the compiler:

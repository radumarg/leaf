...
I want you to generate a lexer using Stefan Höck's idris2-ilex library: https://github.com/stefan-hoeck/idris2-ilex for the Leaf programming language. As a starting point please use the following language specification for Leaf:

// BEGINNING OF LEAF LANGUAGE SYNTAX SPECIFICATION
...
// END OF LEAF LANGUAGE SYNTAX SPECIFICATION

The lexer should start from the following Token.idr and Source.idr files. The Token.idr defines the tokens that the lexer will recognize for the following categories: primitive types, basis states literals, keywords, built-in functions, boolean literals. The Source.idr defines the Located type that will be used by AST after parsing but should not directly impact the lexer since the lexer will produce tokens defined using idris2-parser Bounded data type and the Located type will be generated by the parser after parsing the tokens produced by the lexer:

    lexProgram : String -> Either (Bounded LexerErr) (List (Bounded Token))

// BEGINNING OF Token.idr
...
Here are more requirements for the lexer implementation:

- nested comment should be supported by a custom handler
- skip:
    whitespace
    line comments
    block comments
- keep Rust style documentation comments:
  /// Documentation comment for a function, struct, or module. Leaf supports Rust-style documentation comments.
  /** Documentation comment. */
  //! Documentation comment.
  /*! Documentation comment. */
- Tokens should stay simple, do not make the lexer understand expressions, types, borrow rules, or quantum effects that belongs in the parser/type checker.
- Define keywords and built-in functions before identifiers. Because fn also matches the identifier pattern, keywords must win over identifiers.
- Model attributes lexically, parse structurally. For example for function annotations #[qasm_gate("foo")] the lexer should probably emit something simlar to the following tokens for hash, left bracket, identifier, string literal, right bracket.
- generate Bounded tokens from ilex, these will be convert later by a future parser to my Located AST.
- for nested comments support use state/context support
- make illegal characters explicit, do not silently skip unknown input.
- strings should support escape sequences, including unicode escapes,
- strings should support interpolation: "hello {name}"
- use state/context support for nested comments, string interpolation, and other context-sensitive lexing.
- the lexer implementation must check prefixes in this order:
    1. doc comments and normal comments
    2. basis string: bs"..."
    3. byte string: b"..."
    4. byte char: b'...'
    5. normal string: "..."
    6. normal char: '...'
    7. identifiers/keywords/builtins/types
    8. symbols
- Lexer must check doc comments before normal comments and before /.
- Lexer must support nested block comments.
- Keep unary minus out of numeric literals,  "-7" should be lexed as two tokens: a minus sign and a number, not as a single token.
...
- Identifiers should be lexed as:
  ...
  An apostrophe will not be accepted as the first character of an identifier.
...
- The library runner does not automatically produce your TokEOF; lexProgram must synthesize it after successful scanning.
- Number literals will preserve the exact spelling and must support:
	decimal integers
	binary integers
	octal integers
	...
	exponents
	float suffixes
- use slightly longer more explicit variable and function names as opposed to using ultra compact prefixes and postixes that do not reveal code meaning 
- PLEASE MAKESURE THE GENERATED CODE IS COMMENTED, COMMENTS SHOULD EXPLAIN CODE INTENT
...
LEXER ARCHITECTURE:
  ...
  Frontend/Lexer/
    Regex.idr          -- ilex regex definitions (library of reusable sub-patterns)
    Rules.idr          -- token rules based on tokens declared in Token.idr, this is where pattern -> token rules get assembled
    Errors.idr         -- lexer errors,  should own the translation from ilex's native failure into LexError
    Lexer.idr          -- public lexing API
    Comments.idr       -- explicit module for the comments handler
    

Start from the following code for my Lexer.idr:

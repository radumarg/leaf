...
These are Rust-style functions clauses that specify pre- & post-conditions that the quantum data should satisfy. Exact entanglement tracking at the type level is practical only for restricted fragments or conservative approximations. A fully precise and efficient general-purpose entanglement type system would require classically extracting information from quantum programs that is, in the general case, as hard as the phenomena quantum computers are meant to exploit. When a condition cannot be checked statically a Lean proof obligation will be generated by the compiler:
...
Unlike the unary predicates, product requires minimum two arguments:
...
Stabilized takes as its second argument a list of signed Pauli strings. A more detailed stabilizer contract example:
...
Contracts form two partially ordered sets with `dirty` for unary predicates and respectively `entangled` for n-ary predicates on top:
